...
export async function GET(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const post = await db.query(
      `SELECT p.event_id AS id, p.pubkey AS author, p.author_name, p.title, p.body, p.created_at,
    ...
    const comments = await db.query(
      `SELECT event_id AS id, pubkey AS author, author_name, body, created_at
       FROM comments WHERE root_event_id = $1 AND NOT deleted AND NOT hidden AND created_at > $2
