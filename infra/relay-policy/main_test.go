...
func TestEvaluate(t *testing.T) {
	...
	assertAction(t, evaluate(request("bad-profile", "profile-pubkey", 0, `{"name":"닉네임","about":"no"}`, nil)), "reject")
...
func request(id string, pubkey string, kind int, content string, tags [][]string) Request {
	return Request{
		Type: "new", Event: Event{ID: id, Pubkey: pubkey, Kind: kind, Content: content, Tags: tags},
